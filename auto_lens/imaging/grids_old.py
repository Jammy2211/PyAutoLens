...
def setup_data(mask, data):
    ...
    ----------
    data : ndarray
        A 2D data of data, e.g. the image, signal_to_noise_ratio-map, etc.
    mask : imaging.Mask
        The image mask containing the pixels we are computing the grids of and the image pixel_dimensions / pixel scale.
...
def setup_mapper_2d(mask):
    """ Given an image.Mask, setup an array that can be used to map the input masks grids back to their 2D image \
    pixels.
...
class AnalysisData(object):
    ...
    def __init__(self, mask, image, noise, psf, sub_grid_size=2):
        """The core grouping of lens modeling data, including the image data, signal_to_noise_ratio-map and psf. Optional \
        ...
        noise : image.Noise
            The image signal_to_noise_ratio-map, in variances in electrons per second.
        psf : image.PSF
            The image PSF
        sub_grid_size : int
...
# TODO : This may be where we put our hyper-parameter functions for the image and signal_to_noise_ratio maps. E.g. for signal_to_noise_ratio map \
