...
def setup_sparse_pixels(mask, sparse_grid_size):
    ...
    image_to_sparse : ndarray
        The index of the sparse clustering pixel closes to each image pixel.
    """
...
def setup_image_to_sparse(mask, sparse_mask):
    """For all image pixels in the mask, compute their closest sparse clustering pixel, where the sparse cluster \
    image pixels are the sub-set of image-pixels used within the mask to perform KMeans clustering (this is used \
    purely for speeding up the KMeans clustering algorithim).
