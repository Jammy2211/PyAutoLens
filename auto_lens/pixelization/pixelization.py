...
def kmeans_cluster(coordinates, n_clusters):
    """Perform k-means clustering on a set of image to compute the set of k-means clustering that group the data.
    ...
    ----------
    coordinates : ndarray
        The x and y image to be clustered.
    n_clusters : int
...
def setup_voronoi(coordinates):
    """Setup a Voronoi grid for a given set of image, as well as a list of each Voronoi cell's neighboring \
     Voronoi cells.
    ...
    ----------
    coordinates : ndarray
        The x and y image to derive the Voronoi grid.
    """
...
def sub_coordinates_to_source_pixels_via_nearest_neighbour(sub_coordinates, image_total, sub_total, source_centers):
    """ Match a set of sub image-pixel image to their closest source-pixels, using the source-pixel centers (x,y).

        This method uses a nearest neighbour search between every sub_image-pixel coordinate and set of source-pixel \
        centers, thus it is slow when the number of sub image-pixel image or source-pixels is large. However, it
        is probably the fastest routine for low numbers of sub image-pixels and source-pixels.

        Parameters
        ----------
        sub_coordinates : [(float, float)]
            The x and y sub image-pixel image to be matched to the source-pixel centers.
        image_total : int
            The total number of image pixels in the image.
        sub_total : int
            The total number of sub pixels in the image sub-grid.
        source_centers: [(float, float)
            The source-pixels centers the sub image-pixel image are matched with.

        Returns
        ----------
        image_sub_to_source : [int, int]
            The index in source_pixel_centers each image and sub-image_coordinate is matched with. (e.g. if the fifth
            sub_coordinate of the third image pixel is closest to the 3rd source-pixel in source_pixel_centers,
...
def sub_coordinates_to_source_pixels_via_sparse_pairs(sub_coordinates, image_total, sub_total, source_centers,
    ...
    """ Match a set of sub image-pixel image to their closest source-pixel, using the source-pixel centers (x,y).

        This method uses a sparsely sampled grid of sub image-pixel image with known image-pixel to source-pixel \
        pairings and the source-pixels neighbors to speed up the function. This is optimal when the number of sub \
        image-pixels or source-pixels is large. Thus, the sparse grid of sub must have had a source \
        ...
        In a realistic lens pixelization, the sparse image will correspond to the center of each image pixel \
        (traced to the source-plane) or a sparser grid of image-pixels. The sub will be the sub \
        image-pixels (again, traced to the source-plane). A benefit of this is the source-pixelization (e.g. using \
        KMeans) will be dervied using significantly fewer sub, offering run-time speedup.
        ...
        ----------
        sub_coordinates : [(float, float)]
            The x and y sub to be matched to the source_pixel centers.
        image_total : int
            The total number of image pixels in the image.
        sub_total : int
            The total number of sub pixels in the image sub-grid.
        source_centers: [(float, float)]
            The source_pixel centers the sub are matched with.
        source_neighbors : [[]]
        ...
        sub_to_sparse : [int]
            The index in sub each sparse sub_coordinate is closest too (e.g. if the fifth sparse sub_coordinate \
            is closest to the 3rd sub_coordinate in sub, sub_to_sparse[4] = 2).
...
def find_separation_and_nearest_neighboring_source(sub_coordinate, source_centers, source_neighbors):
    ...
    our input coordinaates.
        ...
        source_centers: [(float, float)
            The source_pixel centers the image are matched with.
...
def compute_squared_separation(coordinate1, coordinate2):
    """Computes the squared separation of two image (no square root for efficiency)"""
