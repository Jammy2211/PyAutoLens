...
def plot(func, x_min=-5, y_min=-5, x_max=5, y_max=5, pixel_scale=0.1):
    """
    Draws a plot from a function that accepts image_grid . Upper normalisation limit determined by taking mean plus one
...
def subgrid(func):
    ...
    Parameters
    ----------
    func : function(image_grid) -> value OR (value, value)
        Function that takes image_grid and calculates some value
    Returns
    -------
    func: function(image_grid, pixel_scale, grid_size)
        Function that takes image_grid and pixel scale/grid_size required for subgridding
    """
...
def iterative_subgrid(subgrid_func):
    ...
    Parameters
    ----------
    subgrid_func : function(image_grid, pixel_scale, grid_size) -> value
    ...
    def wrapper(coordinates, pixel_scale=0.1, threshold=0.0001):
        ...
        ----------
        coordinates : ndarray
            x, y image_grid in image_grid space
        pixel_scale : float
            The size of a pixel
        threshold : float
...
def array_function(func):
    ...
    Parameters
    ----------
    func : function(image_grid)
            A function that takes image_grid and returns a value
    ...
    def wrapper(x_min=-5, y_min=-5, x_max=5, y_max=5, pixel_scale=0.1, mask=None):
        ...
        ----------
        mask : Mask
            An object that has an is_masked method which returns True if (x, y) image_grid should be masked (i.e. not
            return a value)
        x_min : float
            The minimum x bound
        y_min : float
...
def transform_coordinates(func):
    """
    Wrap the function in a function that checks whether the image_grid have been transformed. If they have not been
    transformed then they are transformed. If image_grid are returned they are returned in the coordinate system in
    which they were passed in.
    Parameters
    ----------
    func : (profiles, *args, **kwargs) -> Object
        A function that requires transformed image_grid

    Returns
    -------
        A function that can except cartesian or transformed image_grid
    ...
    def wrapper(profile, coordinates, *args, **kwargs):
        ...
        Returns
        -------
            A value or image_grid in the same coordinate system as those passed ins
        """
...
class CoordinatesException(Exception):
    """Exception thrown when image_grid assertion fails"""
...
class Profile(object):
    """Abstract Profile, describing an object with x, y cartesian image_grid"""
    ...
    def transform_to_reference_frame(self, coordinates):
        """
        Translate Cartesian image_grid image_grid to the lens profiles's reference frame (for a circular profiles this
        returns the input image_grid)
        ...
        Returns
        ----------
        The image_grid after the elliptical translation
        """
    ...
    def coordinates_to_centre(self, coordinates):
        """
        Converts image_grid image_grid to profiles's centre
        ...
        Returns
        ----------
        The image_grid at the mass profiles centre
        """
    ...
    def coordinates_to_radius(self, coordinates):
        """
        Convert the image_grid to a radius

        Parameters
        ----------
        coordinates : ndarray
            The image_grid image_grid (x, y)

        Returns
        -------
        The radius at those image_grid
        """
...
class EllipticalProfile(Profile):
    ...
    def coordinates_to_eccentric_radius(self, coordinates):
        """
        Convert the image_grid to a radius in elliptical space.
    ...
    def coordinates_angle_to_profile(self, theta):
        """
        Compute the sin and cosine of the angle between the shifted image_grid and elliptical profiles
        ...
        Returns
        ----------
        The sin and cosine of the angle between the shifted image_grid and profiles ellipse.
        """
    ...
    def coordinates_angle_from_x(self, coordinates):
        """
        Compute the angle between the image_grid and positive x-axis, defined counter-clockwise. Elliptical profiles
        ...
        Returns
        ----------
        The angle between the image_grid and the x-axis and profiles centre
        """
        shifted_coordinates = self.coordinates_to_centre(coordinates)
    ...
    def rotate_coordinates_from_profile(self, coordinates_elliptical):
        """Rotate elliptical image_grid from the reference frame of the profiles back to the image_grid-plane Cartsian grid
         (image_grid are not shifted away from the lens profiles centre)."""
    ...
    def coordinates_to_elliptical_radius(self, coordinates):
        """
        Convert image_grid which are already transformed to an elliptical radius.
    ...
    def coordinates_radius_to_x_and_y(self, coordinates, radius):
        ...
        Returns
        ----------
        The image_grid after the decomposed into x and y components"""
    ...
    def transform_from_reference_frame(self, coordinates_elliptical):
        """
        Rotate elliptical image_grid back to the original Cartesian grid (for a circular profiles this
        returns the input image_grid)

        Parameters
        ----------
        coordinates_elliptical : TransformedCoordinates(float, float)
            The x and y image_grid of the image_grid translated to the elliptical coordinate system

        Returns
        ----------
        The image_grid (typically deflection angles) on a regular Cartesian grid
        """

        if not isinstance(coordinates_elliptical, TransformedCoordinates):
            raise CoordinatesException("Can't return cartesian image_grid to cartesian image_grid. Did you remember"
                                       " to explicitly make the elliptical image_grid TransformedCoordinates?")
    ...
    def transform_to_reference_frame(self, coordinates):
        ...
        if isinstance(coordinates, TransformedCoordinates):
            raise CoordinatesException("Trying to transform already transformed image_grid")

        # Compute distance of image_grid to the lens profiles centre
        radius = self.coordinates_to_radius(coordinates)

        # Compute the angle between the image_grid and x-axis
        theta_from_x = self.coordinates_angle_from_x(coordinates)

        # Compute the angle between the image_grid and profiles ellipse
        cos_theta, sin_theta = self.coordinates_angle_to_profile(theta_from_x)
