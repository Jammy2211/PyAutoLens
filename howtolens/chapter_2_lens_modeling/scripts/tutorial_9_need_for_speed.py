...
what determines the length of time a __PyAutoLens__ analysis takes, and how one might speed it up.
...
chapter 3 of the __HowToLens__ lectures.
...
I often get asked, given that __PyAutoLens__ is written in Python, isn't it really slow? Afterall, Python is notoriously
slow. Well, no, it isn't. We use a library called 'Numba', that allows us to recompile our Python functions into C
functions before __PyAutoLens__ runs. This gives us C-like speed, but in Python code. If you've got your own code that needs
speeding up, I strongly recommend that you look up Numba:

http://numba.pydata.org/

We've worked very hard to _Profile_ every line of code in __PyAutoLens__ and we're confident its as fast, if not faster,
than any code written in C. In fact, we know this - I wrote the original version of AutoLens in Fortran (bless my
poor soul) and we timed it against __PyAutoLens__. After invoking the magic of Numba, __PyAutoLens__ ran 3 times faster than
...
wrong, go for it - I'll buy you a beer at a conference someday if you can optimize any function in __PyAutoLens__ better
than me!
...
mask out more of the image, we'll fit fewer pixels and __PyAutoLens__ will run faster. Alternatively, we could 'bin-up'
...
non-linear parameter space to be fast, we can  freely butcher our data to make __PyAutoLens__ run even faster! In the
