...
path = "{}".format(os.path.dirname(os.path.realpath(__file__))) # Set the path to the directory of this tutorial
...
# at the edges where neither the lens nor source are visible and the signal is entirely noise..
...
# The shapes of these grids reveals they have been masked:
...
# We used the same tracer to create and fit the image. Therefore, our fit to the image was exceedingly good.
# For example, by inspecting the residuals and chi-squareds, one can see no signs of the source galaxy's light present,
# instead we  just see the noise that we simulated the image with.
...
lens_galaxy = g.Galaxy(mass=mp.EllipticalIsothermal(centre=(0.005, 0.005), einstein_radius=1.6, axis_ratio=0.7,
                                                    phi=45.0))
source_galaxy = g.Galaxy(light=lp.EllipticalSersic(centre=(0.1, 0.1), axis_ratio=0.8, phi=45.0,
                                                        intensity=1.0, effective_radius=1.0, sersic_index=2.5))
tracer = ray_tracing.TracerImageSourcePlanes(lens_galaxies=[lens_galaxy], source_galaxies=[source_galaxy],
...
# Lets change the tracer, one more time, so a solution that is nowhere near the correct one.
...
#    we have no idea what the correct solution is. However might you go about finding the correct solution? Can you
#    find a solution that fits the data reasonable just through trial and error?
