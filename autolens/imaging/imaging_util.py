...
@numba.jit(nopython=True, cache=True)
def image_grid_2d_from_shape_pixel_scales_and_centre(shape, pixel_scales, centre=(0.0, 0.0)):
    """
    ...
    y_cen = float(shape[0] - 1) / 2 - (centre[0] / pixel_scales[0])
    x_cen = float(shape[1] - 1) / 2 + (centre[1] / pixel_scales[1])
...
@numba.jit(nopython=True, cache=True)
def image_grid_1d_from_shape_pixel_scales_and_centre(shape, pixel_scales, centre=(0.0, 0.0)):
    """
    Computes the (y,x) arc second coordinates of every pixel in an datas_ of shape (rows, columns).
...
@numba.jit(nopython=True, cache=True)
def image_grid_1d_masked_from_mask_and_pixel_scales(mask, pixel_scales, centre=(0.0, 0.0)):
    """Compute a 1D grid of (y,x) coordinates, using the center of every unmasked pixel."""

    grid_2d = image_grid_2d_from_shape_pixel_scales_and_centre(mask.shape, pixel_scales, centre)
...
@numba.jit(nopython=True, cache=True)
def sub_grid_1d_masked_from_mask_pixel_scales_and_sub_grid_size(mask, pixel_scales, sub_grid_size, centre=(0.0, 0.0)):
    ...
    y_cen = float(mask.shape[0] - 1) / 2 - (centre[0] / pixel_scales[0])
    x_cen = float(mask.shape[1] - 1) / 2 + (centre[1] / pixel_scales[1])
...
@numba.jit(nopython=True, cache=True)
def grid_arc_seconds_1d_to_grid_pixels_1d(grid_arc_seconds, shape, pixel_scales, centre=(0.0, 0.0)):
    ...
    The arc-second coordinate origin is defined by the class attribute centre, and coordinates are shifted to this \
    ...
    centre : (float, flloat)
        The (y,x) centre of the 2D grid, which the arc-second grid is shifted too.
    """
...
@numba.jit(nopython=True, cache=True)
def grid_arc_seconds_1d_to_grid_pixel_centres_1d(grid_arc_seconds, shape, pixel_scales, centre=(0.0, 0.0)):
    """ Convert a grid of (y,x) arc second coordinates to a grid of (y,x) pixel values. Pixel coordinates are \
...
@numba.jit(nopython=True, cache=True)
def grid_arc_seconds_1d_to_grid_pixel_indexes_1d(grid_arc_seconds, shape, pixel_scales, centre=(0.0, 0.0)):
    ...
    grid_pixels = grid_arc_seconds_1d_to_grid_pixel_centres_1d(grid_arc_seconds=grid_arc_seconds, shape=shape,
                                                               pixel_scales=pixel_scales, centre=centre)
...
@numba.jit(nopython=True, cache=True)
def grid_pixels_1d_to_grid_arc_seconds_1d(grid_pixels, shape, pixel_scales, centre=(0.0, 0.0)):
