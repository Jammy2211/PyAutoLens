...
class Mask(scaled_array.ScaledArray):
    ...
    def map_masked_1d_array_to_2d_array(self, data):
        """ Map a masked 1D array to its original 2D array.
    ...
    def border_sub_pixel_indices(self, sub_grid_size):
        ...
        image-pixel which is closest to the edge."""
...
class ImagingGrids(object):
    ...
    def __init__(self, image, sub, blurring):
        ...
        The image-grid - the (x,y) coordinate at the center of every unmasked pixel.
...
class ImageGrid(np.ndarray):
    ...
    image-pixels. The first element maps of the ndarray corresponds to the unmasked pixel index and second element the \
    ...
    Below is a visual illustration of an image-grid, where a total of 10 pixels are unmasked and are included in the \
    grid.
    ...
    def from_mask(cls, mask):
        """Setup an *ImageGrid* of the regular image-grid from a mask. The center of every unmasked pixel gives \
...
class SubGrid(ImageGrid):
    ...
    image-pixel. Sub-pixel indexes include all previous sub-pixels in all previous unmasked image-pixels.
    ...
    - sub_grid[9, 1] - using a 2x2 sub-grid, gives the 3rd image-pixel's 2nd sub-pixel y-coordinate.
    - sub_grid[9, 1] - using a 3x3 sub-grid, gives the 2nd image-pixel's 1st sub-pixel y-coordinate.
    - sub_grid[27, 0] - using a 3x3 sub-grid, gives the 4th image-pixel's 1st sub-pixel x-coordinate.
    ...
    def grid_from_mask_and_sub_grid_size(cls, mask, sub_grid_size=1):
        """Setup a *SubGrid* of the unmasked image-pixels, using a mask and a specified sub-grid size. The center of \
    ...
    def sub_to_image(self):
        """ Compute the mapping between every sub-pixel and its host unmasked image-pixel.

        For example:

        - sub_to_pixel[8] = 2 -  The seventh sub-pixel is within the 3rd unmasked image pixel.
        - sub_to_pixel[20] = 4 -  The nineteenth sub-pixel is within the 5th unmasked image pixel.
        """
...
class ImagePaddedGrid(ImageGrid):
    ...
    def from_shapes_and_pixel_scale(self, shape, psf_shape, pixel_scale):
        ...
        shape : (int, int)
            The (x,y) shape of the padded-grid's 2D image in units of pixels.
        psf_shape : (int, int)
    ...
    def trim_padded_array_2d_to_original_shape(self, padded_array_2d):
        """Trim a 2d padded array of values (e.g. intensities after PSF blurring) to its original shape.
    ...
    def convolve_padded_array_1d_with_psf_and_trim(self, padded_array_1d, psf):
        ...
        blurred_array_2d = self.trim_padded_array_2d_to_original_shape(blurred_padded_array_2d)
    ...
    def map_padded_1d_array_to_original_2d_array(self, padded_array_1d):
        ...
        """
        return imaging_util.map_unmasked_1d_array_to_2d_array_from_array_1d_and_shape(padded_array_1d, self.original_shape)
...
class SubPaddedGrid(SubGrid, ImagePaddedGrid):
    ...
    def __init__(self, arr, mask, original_shape, padded_shape, sub_grid_size=1):
        ...
        opposed to just within the masked region.
        """
        super(SubPaddedGrid, self).__init__(arr, mask, sub_grid_size)
    ...
    def from_mask_sub_grid_size_and_psf_shape(cls, mask, sub_grid_size, psf_shape):
        ...
        padded_sub_grid = imaging_util.sub_grid_1d_masked_from_mask_pixel_scale_and_sub_grid_size(mask=np.full(padded_shape, False),
...
class ImagingGridBorders(object):
    ...
    def __init__(self, image, sub):
        ...
        The image-border - The pixel-indexes of all unmasked pixels in an *ImageGrid* that are border-pixels.
        ...
        image : ImageGridBorder
            The pixel-index's of all image-pixels which are on the mask border.
        sub : SubGridBorder
    ...
    def relocated_grids_from_grids(self, grids):
        ...
        -----------
        grids : ImagingGrids
            The imaging-grids (image, sub) which have their coordinates relocated.
        """
...
class ImageGridBorder(np.ndarray):
    ...
    def __new__(cls, arr, polynomial_degree=3, centre=(0.0, 0.0), *args, **kwargs):
        """The borders of an image-grid, containing the pixel-index's of all unmasked pixels that are on the \
    ...
    def grid_to_radii(self, grid):
        ...
        ----------
        grid : ImageGrid
            The (x, y) coordinates of each unmasked image-pixel.
        """
