...
class PositionsSolver:
    ...
    def image_plane_positions_from(self, lensing_obj, source_plane_coordinate):
        ...
        deflections = lensing_obj.deflections_from_grid(grid=self.grid)
        source_plane_grid = self.grid.grid_from_deflection_grid(deflection_grid=deflections)
        ...
        trough_pixels = trough_pixels_from(
            array_2d=source_plane_squared_distances.in_2d, mask=self.grid.mask
        )
        ...
        for x in range(1, grid_2d.shape[1] - 1):
            if not mask[y, x] and not mask_util.check_if_edge_pixel(
                ...
                if (
                    ...
                    print(y, x, top_left_quadrant, top_right_quadrant, bottom_left_quadrant, bottom_right_quadrant)
