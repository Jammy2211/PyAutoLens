...
class AbstractGriddedPlane(AbstractPlane):
    def __init__(
        ...
        if compute_deflections:
            ...
            def calculate_deflections(grid):
                ...
                if galaxies:
                    return sum(
                        map(lambda galaxy: galaxy.deflections_from_grid(grid), galaxies)
                    )
                else:
    ...
    def profile_image_plane_image(self, return_in_2d=True, return_binned=True):
        ...
        if self.galaxies:
            return sum(
                map(
                    lambda g: g.intensities_from_grid(grid=self.grid_stack.sub),
                    self.galaxies,
                )
            )
        else:
    ...
    def profile_image_plane_blurring_image(self, return_in_2d=True):
        ...
        if self.galaxies:
            return sum(
                map(
                    lambda g: g.intensities_from_grid(grid=self.grid_stack.blurring),
                    self.galaxies,
                )
            )
        else:
