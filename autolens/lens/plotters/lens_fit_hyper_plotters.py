...
def plot_fit_subplot(
        fit_hyper, fit, should_plot_mask=True, extract_array_from_mask=False, zoom_around_mask=False, positions=None, 
    ...
    if fit.tracer.total_planes == 1:
        ...
        plot_fit_subplot_lens_plane_only(
            fit_hyper=fit_hyper, fit=fit, should_plot_mask=should_plot_mask, extract_array_from_mask=extract_array_from_mask, zoom_around_mask=zoom_around_mask, 
...
def plot_fit_subplot_lens_plane_only(
    ...
    lens_plotter_util.plot_image(
        fit=fit_hyper, mask=mask, extract_array_from_mask=extract_array_from_mask, zoom_around_mask=zoom_around_mask, positions=positions,
    ...
    lens_plotter_util.plot_model_data(
        fit=fit_hyper, mask=mask, extract_array_from_mask=extract_array_from_mask, zoom_around_mask=zoom_around_mask, as_subplot=True,
         units=units, kpc_per_arcsec=kpc_per_arcsec, figsize=figsize, aspect=aspect,
         cmap=cmap, norm=norm, norm_min=norm_min, norm_max=norm_max, linthresh=linthresh, linscale=linscale,
         cb_ticksize=cb_ticksize, cb_fraction=cb_fraction, cb_pad=cb_pad, 
        cb_tick_values=cb_tick_values, cb_tick_labels=cb_tick_labels,
...
def plot_fit_individuals(fit_hyper, fit, should_plot_mask=True, extract_array_from_mask=False, zoom_around_mask=False,
                         positions=None,
                         should_plot_image_plane_pix=False,
    ...
    if fit.tracer.total_planes == 1:
        ...
        plot_fit_individuals_lens_plane_only(
            fit_hyper=fit_hyper, fit=fit, should_plot_mask=should_plot_mask,
...
def plot_fit_individuals_lens_plane_only(
    ...
    if should_plot_image:
        ...
        lens_plotter_util.plot_image(
            fit=fit_hyper, mask=mask, extract_array_from_mask=extract_array_from_mask,
