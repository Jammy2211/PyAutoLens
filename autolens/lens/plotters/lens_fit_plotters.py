...
def plot_fit_individuals(
        ...
        should_plot_pixelization_residuals=False,
        should_plot_pixelization_normalized_residuals=False,
        should_plot_pixelization_chi_squareds=False,
    ...
    if should_plot_pixelization_residuals:

        if fit.total_inversions == 1:

            inversion_plotters.plot_pixelization_residuals(
                inversion=fit.inversion, should_plot_grid=True,
                units=units, figsize=(20, 20),
                output_path=output_path, output_format=output_format)

    if should_plot_pixelization_normalized_residuals:

        if fit.total_inversions == 1:

            inversion_plotters.plot_pixelization_normalized_residuals(
                inversion=fit.inversion, should_plot_grid=True,
                units=units, figsize=(20, 20),
                output_path=output_path, output_format=output_format)

    if should_plot_pixelization_chi_squareds:

        if fit.total_inversions == 1:

            inversion_plotters.plot_pixelization_chi_squareds(
                inversion=fit.inversion, should_plot_grid=True,
                units=units, figsize=(20, 20),
                output_path=output_path, output_format=output_format)

    if should_plot_pixelization_regularization_weights:
