...
def source_only_pipeline(image, mask):
    """
    Fit a source-only masked_image (i.e. no lens light component)

    Parameters
    ----------
    image: Image
        An masked_image of a lens galaxy including metadata such as PSF, background noise and effective exposure time
    mask: Mask
        A mask describing which parts of the masked_image should be excluded from the analysis
    ...
    logger.info(
        ...
        Image: Observed masked_image used throughout.
    ...
    # Create an analysis object for the masked_image and mask
...
def lens_and_source_pipeline(image, lens_mask, source_mask, combined_mask):
    ...
    - Draw annulus for mask containing the source (excludes central regions of masked_image where lens is).
    """
    ...
    1) Image: Observed masked_image, includes lens+source.
       Mask: Circle / Ellipse containing entire lens galaxy.
       Light: Sersic (This phase simply subtracts the lens light from the masked_image - the source is present and disrupts the 
       NLO: LM        fit but we choose not to care)
       Purpose: Provides lens subtracted masked_image for subsequent phases.

    """
    # Create an analysis object for the masked_image and a mask that means only lens light is visible
    ...
    """
    2) Image: The lens light subtracted masked_image from phase 1.
    ...
    """
    3) Image: The lens light subtracted masked_image from phase 1.
       Mask: Circle / Ellipse containing entire lens galaxy.
       Light: None
    ...
    """
    4) Image: Observed masked_image, includes lens+source.
       Mask: Circle / Ellipse containing entire lens galaxy.
...
def profiles_pipeline(paths, image, mask):
    """
     Fit an masked_image with profiles, gradually adding extra profiles.
...
# def profiles_pipeline(paths, masked_image, mask):
#     """
#      Fit an masked_image with profiles, gradually adding extra profiles.
#
#      Parameters
...
#
#     # Create an analysis object for the masked_image and mask
#     analysis = an.Analysis(masked_image, mask)
#
#     logger.info(
#         """
#         1) Light: Elliptical Sersic x1
