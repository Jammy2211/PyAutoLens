    ...
    def profile_quantity_from_galaxy_and_sub_grid(self, galaxies, sub_grid):
        ...
        if self.use_intensities:
            return sum(map(lambda g: g.intensities_from_grid(
                grid=self.grid_stack.sub, return_in_2d=False, return_binned=True), galaxies))
        elif self.use_convergence:
            return sum(map(lambda g: g.convergence_from_grid(
                grid=self.grid_stack.sub.unlensed_grid_1d, return_in_2d=False, return_binned=True), galaxies))
            ...
            return sum(map(lambda g: g.deflections_from_grid(
                grid=self.grid_stack.sub.unlensed_grid_1d, return_in_2d=False, return_binned=True), galaxies))[:, 0]
        elif self.use_deflections_x:
            return sum(map(lambda g: g.deflections_from_grid(
                grid=self.grid_stack.sub.unlensed_grid_1d, return_in_2d=False, return_binned=True), galaxies))[:, 1]
