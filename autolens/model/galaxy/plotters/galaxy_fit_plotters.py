...
def plot_single_subplot(
    ...
    plot_galaxy_data_array(
        galaxy_data=fit.galaxy_data, mask=mask, extract_mask_region=extract_mask_region, positions=positions,
        as_subplot=True,
...
def plot_fitting_individuals(fit, should_plot_mask=True, extract_mask_region=False, positions=None,
                            units='kpc',
                            output_path=None, output_format='show', ignore_config=False):

    if not ignore_config:

        plot_galaxy_fitting_model_image = \
            conf.instance.general.get('output', 'plot_galaxy_fitting_model_image', bool)
        plot_galaxy_fitting_residual_map = \
            conf.instance.general.get('output', 'plot_galaxy_fitting_residual_map', bool)
        plot_galaxy_fitting_chi_squared_map = \
            conf.instance.general.get('output', 'plot_galaxy_fitting_chi_squared_map', bool)

    else:

        plot_galaxy_fitting_model_image = True
        plot_galaxy_fitting_residual_map = True
        plot_galaxy_fitting_chi_squared_map = True
...
def plot_galaxy_data_array(
        galaxy_data, mask=None, extract_mask_region=False, positions=None, as_subplot=False,
        units='arcsec', kpc_per_arcsec=None, figsize=None, aspect='equal',
        cmap='jet', norm='linear', norm_min=None, norm_max=None, linthresh=0.05, linscale=0.01,
        cb_ticksize=10, cb_fraction=0.047, cb_pad=0.01,
        titlesize=10, xlabelsize=10, ylabelsize=10, xyticksize=10,
    ...
    array_plotters.plot_array(
        array=galaxy_data.image, mask=mask, extract_mask_region=extract_mask_region, positions=positions,
