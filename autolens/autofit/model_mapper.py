...
class ModelMapper(AbstractModel):
    ...
    def __init__(self, config=None, width_config=None, **classes):
        ...
        # All class instances that are to be generated by the collection are specified by passing their analysis_path and
        # class:

        collection.add_class("sersic_1", light_profile.EllipticalSersicMP)
        collection.add_class("sersic_2", light_profile.EllipticalSersicMP)
        ...
        # But this attribute is an instance of the actual EllipticalSersicMP class

        # A ModelMapper can be concisely constructed using keyword arguments:

        collection = prior.ModelMapper(config, source_light_profile=light_profile.EllipticalSersicMP,
