...
class Transformer(object):
    ...
    def real_visibilities_from_intensities(
        ...
        if self.preload_transform:
            ...
            return self.real_visibilities_via_preload_jit(intensities_1d=intensities_1d,
    ...
    def preload_real_transforms(grid_radians, uv_wavelengths, total_image_pixels):
        ...
        preloaded_real_transforms = np.zeros(shape=(total_image_pixels, uv_wavelengths.shape[0]))

        for i in range(total_image_pixels):
            for j in range(uv_wavelengths.shape[0]):
                preloaded_real_transforms[i,j] += np.cos(-2.0 * np.pi * (
    ...
    def imaginary_visibilities_from_intensities(
            self, intensities_1d,
    ):

        if self.preload_transform:

            return self.imaginary_visibilities_via_preload_jit(intensities_1d=intensities_1d,
    ...
    def preload_imaginary_transforms(grid_radians, uv_wavelengths, total_image_pixels):
        ...
        preloaded_imaginary_transforms = np.zeros(shape=(total_image_pixels, uv_wavelengths.shape[0]))

        for i in range(total_image_pixels):
            for j in range(uv_wavelengths.shape[0]):
                preloaded_imaginary_transforms[i,j] += np.sin(-2.0 * np.pi * (
                grid_radians[i, 1] * uv_wavelengths[j, 0] - grid_radians[i, 0] * uv_wavelengths[j,1]))
    ...
    @staticmethod
    @decorator_util.jit()
    def imaginary_visibilities_via_preload_jit(intensities_1d, preloaded_imaginarys, total_visibilities, total_image_pixels):

        imaginary_visibilities = np.zeros(shape=(total_visibilities))

        for i in range(total_image_pixels):
            for j in range(total_visibilities):
                imaginary_visibilities[j] += intensities_1d[i] * preloaded_imaginarys[i,j]

        return imaginary_visibilities

    @staticmethod
    @decorator_util.jit()
    def imaginary_visibilities_jit(intensities_1d, grid_radians, uv_wavelengths, total_visibilities, total_image_pixels):

        imaginary_visibilities = np.zeros(shape=(total_visibilities))

        for i in range(total_image_pixels):
            for j in range(total_visibilities):
                imaginary_visibilities[j] += intensities_1d[i] * np.sin(-2.0 * np.pi * (
                        grid_radians[i, 1] * uv_wavelengths[j, 0] - grid_radians[
                    i, 0] * uv_wavelengths[j, 1]))

        return imaginary_visibilities

    def visibilities_from_intensities(self, intensities_1d):

        real_visibilities = self.real_visibilities_from_intensities(intensities_1d=intensities_1d)
        imaginary_visibilities = self.imaginary_visibilities_from_intensities(intensities_1d=intensities_1d)
