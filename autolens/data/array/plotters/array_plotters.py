...
def plot_array(array, origin=None, mask=None, should_plot_border=False, positions=None, grid=None, as_subplot=False,
    ...
    """Plot an array of datas as a figure.

    Parameters
    -----------
    array : ndarray or datas.array.scaled_array.ScaledArray
        The 2D array of datas which is plotted.
    origin : (float, float).
        The origin of the coordinate system of the datas, which is plotted as an 'x' on the datas if input.
    mask : ndarray of datas.array.masks.Mask
        The masks applied to the datas, the edge of which is plotted as a set of points over the plotted array.
    should_plot_border : bool
        If a masks is supplied, its borders pixels (e.g. the exterior edge) is plotted if this is *True*.
    positions : [[]]
        Lists of (y,x) coordinates on the datas which are plotted as colored dots, to highlight specific pixels.
    ...
    aspect : str
        The aspect ratio of the datas, specifically whether it is forced to be square ('equal') or adapts its size to \
        the figure size ('auto').
    cmap : str
        The colormap the array is plotted using, which may be chosen from the standard matplotlib colormaps.
    norm : str
        The normalization of the colormap used to plot the datas, specifically whether it is linear ('linear'), log \
...
def plot_figure(array, as_subplot, units, kpc_per_arcsec, figsize, aspect, cmap, norm, norm_min, norm_max,
    ...
    """Open a matplotlib figure and plot the array of datas on it.
...
def get_normalization_min_max(array, norm_min, norm_max):
    ...
    If norm_min / norm_max are not supplied, the minimum / maximum values of the array of datas are used.
...
def convert_grid_units(array, grid_arc_seconds, units, kpc_per_arcsec):
    ...
    -----------
    array : ndarray or datas.array.scaled_array.ScaledArray
        The 2D array of datas which is plotted, the shape of which is used for converting the grid to units of pixels.
    grid_arc_seconds : ndarray or datas.array.grid_stacks.RegularGrid
...
def plot_border(mask, should_plot_border, units, kpc_per_arcsec, pointsize):
    ...
    Parameters
    -----------t.
    masks : ndarray of datas.array.masks.Mask
        The masks applied to the datas, the edge of which is plotted as a set of points over the plotted array.
    should_plot_border : bool
        If a masks is supplied, its borders pixels (e.g. the exterior edge) is plotted if this is *True*.
    units : str
...
def plot_points(points_arc_seconds, array, units, kpc_per_arcsec, pointsize):
    """Plot a set of points over the array of datas on the figure.
...
def plot_grid(grid_arc_seconds, array, units, kpc_per_arcsec, pointsize):
    """Plot a grid of points over the array of datas on the figure.
