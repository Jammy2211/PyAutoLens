...
def plot_reconstructed_pixelization(inversion, mask=None, positions=None, should_plot_centres=False,
                                    should_plot_grid=False, image_pixels=None, source_pixels=None,
                                    units='arcsec', kpc_per_arcsec=None, figsize=(7, 7), aspect='equal',
    ...
    tools.setup_figure(figsize=figsize, as_subplot=False)

    reconstructed_pixelization = \
        inversion.mapper.reconstructed_pixelization_from_solution_vector(inversion.solution_vector)

    tools_array.plot_array(array=reconstructed_pixelization, mask=mask, positions=positions, as_subplot=True,
                           units=units, kpc_per_arcsec=kpc_per_arcsec, figsize=figsize, aspect=aspect,
                           cmap=cmap, norm=norm, norm_min=norm_min, norm_max=norm_max,
                           linthresh=linthresh, linscale=linscale,
                           cb_ticksize=cb_ticksize, cb_fraction=cb_fraction, cb_pad=cb_pad,
    ...
    mapper_plotters.plot_rectangular_mapper(mapper=inversion.mapper, should_plot_centres=should_plot_centres,
                                            ...
                                            image_pixels=image_pixels, source_pixels=source_pixels, as_subplot=True,
                                            units='arcsec', kpc_per_arcsec=kpc_per_arcsec)

    tools.output_figure(array=reconstructed_pixelization, as_subplot=False, output_path=output_path,
                        output_filename=output_filename, output_format=output_format)
    tools.close_figure(as_subplot=False)
