...
class Regularization(object):
    ...
    def __init__(self, coefficients=(1.0,)):
        ...
        linearly invert and fit the _image data. Thus, it changes the pixelization in a linear manner, ensuring that \
...
class Weighted(Regularization):
    ...
    def pixel_signals_from_images(self, pixels, image_to_pixelization, galaxy_image):
        """Compute the (scaled) signal in each pixel, where the signal is the sum of its _image-pixel fluxes. \
        ...
        1) Divided by the number of _image-pixels in the pixel, to ensure all pixels have the same \
        ...
        regularizations weights are defined identically for any _image units or signal-to-noise ratio.
