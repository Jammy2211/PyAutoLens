...
class FitPositionsImagePairAll(AbstractFitPositionsImagePair):
    ...
    def all_permutations_log_likelihoods(self) -> np.ndarray:
        ...
        """
        is_nan = jnp.isnan(self.model_data.array).any(axis=1)
        sentinel = jnp.full_like(self.model_data.array[0], fill_value=jnp.inf)
        model_data = jnp.where(is_nan[:, None], sentinel, self.model_data.array)
