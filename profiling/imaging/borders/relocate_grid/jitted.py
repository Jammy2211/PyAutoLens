...
assert (lsst_border.relocated_grid_from_grid(grid=lsst.grids.sub) ==
        pytest.approx(lsst_border.relocated_grid_from_grid_jitted(grid=lsst.grids.sub), 1e-4))
...
@tools.tick_toc_x1
def lsst_solution():
    lsst_border.relocated_grid_from_grid_jitted(grid=lsst.grids.sub)


@tools.tick_toc_x1
def euclid_solution():
    euclid_border.relocated_grid_from_grid_jitted(grid=euclid.grids.sub)


@tools.tick_toc_x1
def hst_solution():
    hst_border.relocated_grid_from_grid_jitted(grid=hst.grids.sub)


@tools.tick_toc_x1
def hst_up_solution():
    hst_up_border.relocated_grid_from_grid_jitted(grid=hst_up.grids.sub)


@tools.tick_toc_x1
def ao_solution():
    ao_border.relocated_grid_from_grid_jitted(grid=ao.grids.sub)
