...
class Convolver(object):
    ...
    def setup_image_frames(self):
        ...
        for x in range(self.mask_index_array.shape[0]):
            for y in range(self.mask_index_array.shape[1]):
                if not self.mask[x][y]:
                    image_frame_indexes, image_frame_kernels = \
                        self.frame_at_coords_jit((x, y), self.mask, self.mask_index_array, self.kernel,
                                                 self.kernel_shape)
    ...
    def frame_at_coords_jit(coords, mask, mask_index_array, kernel, kernel_shape):
        """
        Parameters
        ----------
        coords: (int, int)
