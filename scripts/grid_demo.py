...
# grid_coords, which is an abstract object containing a set of coordinates which we will then pass through the
# ray-tracing module.
...
# We can have multiple grids (which are different to the image_coords grid_coords we're using here). Thus, before ray-tracing,
...
# We'll assume only one image_coords-plane and one source-plane (multiple planes will be added to the code in the future).
ray_trace = ray_tracing.Tracer(lens_galaxies=[lens_galaxy], source_galaxies=[source_galaxy],
                               image_plane_grids=grid_collection)

# The ray tracing sets up an image_coords plane, whose coordinates are our original coordinates.
print('--- VALUES OF IMAGE COORDINATES, DEFLS AND SOURCE COORDINATES ---')
print('image_coords plane grid_coords coordinates = ', ray_trace.image_plane.grids.image)

# The image_coords plane is also automatically set up with deflection angles, using the lens galaxy mass profile:
print('image_coords plane deflection angles = ', ray_trace.image_plane.deflections.image)

# And a source plane is set up too, which is the image_coords plane coordinates - the image_coords plane deflection angles
print('source plane grid_coords coordinates =', ray_trace.source_plane.grids.image, '\n')

# If we pass the same lens galaxy to the image_coords plane 3 times, notice that the deflection angles triple (as we are
...
ray_trace_x3 = ray_tracing.Tracer(lens_galaxies=[lens_galaxy, lens_galaxy, lens_galaxy],
                                  source_galaxies=[source_galaxy], image_plane_grids=grid_collection)
print('image_coords plane x3 grid_coords coordinates = ', ray_trace_x3.image_plane.grids.image)
print('image_coords plane x3 deflection angles = ', ray_trace_x3.image_plane.deflections.image)
print('source plane x3 grid_coords coordinates =', ray_trace_x3.source_plane.grids.image, '\n')

# We can now go to any plane, and compute a model image_coords of the galaxy(s) in that plane using their light profiles.
galaxy_image_plane = ray_trace.image_plane.generate_image_of_galaxy_light_profiles()
print('--- PLANE GALAXY IMAGES ---')
print('galaxy image_coords (intensities) from image_coords plane = ', galaxy_image_plane)
...
# We can also do this for the source-plane and for the entire ray-tracing plane (for the image_coords of all galaxies):
...
print('galaxy image_coords (intensities) from source plane = ', galaxy_source_plane)
print('galaxy image_coords (intensities) from all of ray-tracing (e.g. image_coords plane + source plane) = ', galaxy_ray_trace)

# We will also be able to attach a pixelization to a galaxy, which is used to reconstruct the image_coords via a pixel
# grid_coords. source_galaxy = galaxy.Galaxy(pixelization=AdaptivePixelization)
...
# desire. These will then be used to fit in the image_coords weighted_data in what will be the 'analysis' module.


### POTENTIAL GRID ###

# The potential grid_coords is basically a rectangular grid_coords of coordinates (e.g. of size (20, 20)). We could
# store these coordinates as another attribute of the GridCoordsCollection class, such that when we call
# TraceImageAndSource, the image_plane has an additional set of coordinates ray_trace.image_coords.plane.grids.potential_grid.
# In the image_coords plane, the galaxy potential at these coordinates will be automatically computed.

# The potential grid_coords is computed in the image_coords-plane, so it doesn't need to know the traced image_coords coordinates in
# the source-plane. Thus, we'll make sure the potential grid_coords coordinates do not have their deflection angles
# computed and that they are not traced to the source plane.

# Now our image_coords plane knows the potential grid_coords coordinates (and the galaxies in image_coords plane, from which to
# compute their potentials), We could extract the potential grid_coords much like the model images / pixelization above:
# e.g. ray_trace.compute_potential_grid()

# My guess is that this will do something along the lines of:

# 1) Using the image_coords plane, compute the potential on the potential_grid.
