...
# grid, which is an abstract object containing a set of coordinates which we will then pass through the ray-tracing
# module.
...
# We can have multiple grids (which are different to the image grid we're using here). Thus, before ray-tracing, we
# combine all of our grids into a grid collection. This doesn't change any of aspect of this tutorial.
grid_collection = grids.GridCollection(image=image_grid)
print('--- VALUES OF GRID COLLECTION COORDINATES [(1.0, 1.0), (2.0, 2.0)] ---')
print('grid collection coordinates = ', grid_collection.image.grid, '\n')

# Now lets pass our lens galaxy, source galaxy and grid collection through the ray tracing module.
