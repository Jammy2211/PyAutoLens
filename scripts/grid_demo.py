...
# We will also be able to attach a pixelization to a galaxy, which is used to reconstruct the image via a pixel
...
# TraceImageAndSource, the image_plane has an additional set of coordinates ray_trace.image.plane.grids.potential_grid.
# In the image plane, the galaxy potential at these coordinates will be automatically computed.

# The potential grid_coords is computed in the image-plane, so it doesn't need to know the traced image coordinates in
...
# Now our image plane knows the potential grid_coords coordinates (and the galaxies in image plane, from which to
