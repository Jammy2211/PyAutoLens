...
class TestAbstractPhaseVisualizer:
    ...
    def test__visualizer_with_preloaded_critical_curves_and_caustics_is_setup(
            self, masked_imaging_7x7, tracer_x2_plane_7x7, include_all, plot_path, plot_patch
    ):
...
class TestPhaseDataSetVisualizer:
    ...
    def test__visualizes_ray_tracing_using_configs(
       self,  masked_imaging_7x7, tracer_x2_plane_7x7, include_all, plot_path, plot_patch
        ...
        assert plot_path + "subplots/subplot_tracer.png" in plot_patch.paths
        assert (
            plot_path + "ray_tracing/profile_image.png" in plot_patch.paths
        )
        assert (
            plot_path + "ray_tracing/source_plane.png" in plot_patch.paths
        )
        assert (
            plot_path + "ray_tracing/convergence.png"
            in plot_patch.paths
        )
        assert plot_path + "ray_tracing/potential.png" not in plot_patch.paths
        assert (
            plot_path + "ray_tracing/deflections_y.png"
            not in plot_patch.paths
        )
        assert (
            plot_path + "ray_tracing/deflections_x.png"
            not in plot_patch.paths
        )
        assert (
            plot_path + "ray_tracing/magnification.png"
            in plot_patch.paths
        )
...
class TestPhaseImagingVisualizer:
    ...
    def test__visualizes_imaging_using_configs(
        ...
        assert (
            plot_path + "imaging/signal_to_noise_map.png"
            not in plot_patch.paths
        )
        assert (
        ...
        assert (
            plot_path + "imaging/potential_chi_squared_map.png"
            in plot_patch.paths
        )


    def test__source_and_lens__visualizes_fit_using_configs(
        self, masked_imaging_7x7, masked_imaging_fit_x2_plane_7x7, include_all, plot_path, plot_patch
    ):
        ...
        assert (
            plot_path + "fit_imaging/signal_to_noise_map.png" not in plot_patch.paths
        )
        ...
        assert (
            plot_path + "fit_imaging/normalized_residual_map.png" in plot_patch.paths
        )
        ...
        assert (
            plot_path + "fit_imaging/model_image_of_plane_0.png"
            not in plot_patch.paths
        )
        assert (
            plot_path + "fit_imaging/model_image_of_plane_1.png"
            not in plot_patch.paths
        )
    ...
    def test__visualizes_hyper_images_using_config(
        self, masked_imaging_7x7, hyper_model_image_7x7, include_all, hyper_galaxy_image_path_dict_7x7, plot_path, plot_patch,
    ):
...
class TestPhaseInterferometerVisualizer:
    ...
    def test__visualizes_interferometer_using_configs(
        self, masked_interferometer_7, general_config, include_all, plot_path, plot_patch
    ):
        ...
        assert (
            plot_path + "interferometer/visibilities.png"
            in plot_patch.paths
        )
        assert (
            plot_path + "interferometer/u_wavelengths.png"
            not in plot_patch.paths
        )
        assert (
            plot_path + "interferometer/v_wavelengths.png"
            not in plot_patch.paths
        )
        assert (
            plot_path + "interferometer/primary_beam.png"
            in plot_patch.paths
        )

    def test__source_and_lens__visualizes_fit_using_configs(
        self, masked_interferometer_7, masked_interferometer_fit_x2_plane_7x7, include_all, plot_path, plot_patch
    ):
        ...
        assert (
            plot_path + "fit_interferometer/signal_to_noise_map.png" not in plot_patch.paths
        )
        assert plot_path + "fit_interferometer/model_visibilities.png" in plot_patch.paths
        assert (
            plot_path + "fit_interferometer/residual_map_vs_uv_distances_real.png"
            not in plot_patch.paths
        )
        assert (
            plot_path + "fit_interferometer/normalized_residual_map_vs_uv_distances_real.png"
            in plot_patch.paths
        )
        assert (
...
class TestHyperGalaxyVisualizer:
    ...
    def test__hyper_fit__images_for_phase__source_and_lens__depedent_on_input(
        self, masked_imaging_fit_x2_plane_7x7, hyper_galaxy_image_0_7x7, include_all, plot_path, plot_patch
    ):
        ...
        assert (
            plot_path + "subplots/subplot_fit_hyper_galaxy.png" in plot_patch.paths
        )