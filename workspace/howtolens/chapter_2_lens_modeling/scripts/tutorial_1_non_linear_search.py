...
# In this example, we're going to take an images and find a lens model that provides a good fit to it and we're going
...
# represent lens model_galaxy, source model_galaxy and therefore create a tracer. Thus, to begin, we have to choose the
...
# I'll let you into a secret - this is the same lens model that I used to simulate the images we'll fit (but I'm not
...
# random set of parameters. We could use this lens model to create a tracer and fit the images-datas, and quantify how
# good the fit was using its likelihood (we inspected this in previous tutorial). If we kept guessing lens
# models, eventually we'd find one that provides a good fit (i.e. high likelihood) to the datas!
...
# good fit to the datas, another set with similar values probably will too.
...
# 2) Pass this tracer through the fitting module, generating a model images and comparing this model images to the
...
# This function simulates the images we'll fit in this tutorial.
def simulate():
...
# and this calls the function, setting us up with an images to model. Lets plot it
image = simulate()
...
# To run the phase, we simply pass it the images datas we want to fit, and the non-linear search begins! As the phase
...
# The fit looks good, and we've therefore found a model pretty close to the one we simulated the images with (you can
