...
# The goal of this, rather short, exercise, is to fit this 'realistic' model to a simulated images, where the lens's
...
# Another simulate images function, albeit it generates a new images
def simulate():
...
# When we plot it, the lens light's is clealy visible in the centre of the images
...
# And lets look at the images.
lensing_fitting_plotters.plot_fitting_subplot(fit=results.fit)

# Uh-oh. That images didn't look very good, did it? If we compare our inferred parameters to the actual values (in the
...
# Create a lensing images to make the fit - the masks we used above was a 3" circle (we'll come back to this later)
...
# Make the tracer we use to simulate the images
lens_galaxy = g.Galaxy(light=lp.EllipticalSersic(centre=(0.0, 0.0), axis_ratio=0.9, phi=45.0, intensity=0.04,
                                                         effective_radius=0.5, sersic_index=3.5),
                       mass=mp.EllipticalIsothermal(centre=(0.0, 0.0), axis_ratio=0.8, phi=45.0, einstein_radius=0.8))
...
# Now, lets fit the lensing images with the tracer and plot the fit. It looks a lot better than above, doesn't it?
...
# 1) When you look at an images of a strong lens, do you get a sense of roughly what values certain lens model
#    parameters are?
