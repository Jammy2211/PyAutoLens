...
# inversion  reconstructs a source, it doesn't *just* compute the set of fluxes that best-fit_normal the image. It is also
...
# the image. More specifically, we over-fit_normal the noise in the image - which is what the the large flux values located
...
# strong lens. By smoothing our source reconstruction, we ensure it doesn't fit_normal the noise in the image. If we set a
...
# that best-fit_normal the image *including* the penalty term due to comparing neighboring source-pixel fluxes.
...
#    provide a higher evidence, provided it still has enough resolution to fit_normal the image well (and provided that the
