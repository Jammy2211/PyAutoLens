...
# Next, lets set this images up as a lensing images, and setup a tracer using the input lens model_galaxy model (we don't need
...
# And now, finally, we're going to use our mapper to invert the images using the 'inversions' module, which is imported
...
# Our inversion has a reconstructed images and pixeilzation, whcih we can plot using an inversion plotter
...
# This code is doing all the the same as above (setup the images, galaxies, tracer, mapper, ec.),
...
# Pretty great, huh? If you ran the complex source pipeline, you'll remember that getting a model images that looked
...
# To begin, lets consider some random mappings between our mapper's source-pixels and the images
...
# 1) The mappings between every source-pixel and a set of images-pixels.
# 2) The flux values in every observed images-pixel, which are the values we want to fit successfully.

# It turns out that with these two pieces of information we can linearly solve for the set of source-pixel fluxes that
# best-fit (e.g. maximize the likelihood of) our observed images. Essentially, we set up the mapping between source and
# images pixels as a large matrix, and solve for the source-pixel fluxes in an analogous fashion to how you would
...
# 1) We've discussed the images sub-grid before, which splits each images-pixel into a sub-pixel. If a sub-grid is used,
#    it is the mapping between every sub-pixel and source-pixel that is computed and used to perform the inversion.
#    This prevents aliasing effects degrading the images reconstruction, and, as a rule of thumb, I would
#    suggest you use sub-gridding of degree 2x2.

# 2) When fitting images's using light profiles, we discussed how a 'model_image' was generated by blurring them with
#    the instrument's PSF. A similar blurring operation is incorporated into the inversion, such that the reconstructed
#    images and source fully account for the telescope optics and effect of the PSF.

# 3) The inversion's solution is regularized. But wait, that's what we'll cover in the next tutorial!


# Finally, let me show you how easy it is to fit an images with an inversion using the fitting module. Instead of giving
...
# Then, like before, we call on the fitting module to perform the fit to the lensing images. Indeed, we see
...
# 1) The inversion provides the best-fit solution to the observed images. Is there a problem with seeking the 'best-fit'?
#    Is there a risk that we're going to fit other things in the images than just the lensed source model_galaxy? What happens
#    if you reduce the regularization 'coefficient' above to zero?

# 2) The exterior pixels in the rectangular grid have no images-pixels in them. However, they are still given a
#    reconstructed flux. If this value isn't' coming from a mapping to an images-pixel, where is it be coming from?
