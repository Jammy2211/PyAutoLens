...
# Phase 4) Refit the lens galaxy's light and mass models using an inversion, with lens galaxy priors initialized from
#          phases 1 and 2 and source-pixelization parameters from phase 3.

# The first 2 phases of this pipeline are identical to the 'lens_light_and_x1_source_parametric.py' pipeline.
...
def make_lens_light_and_x1_source_inversion_pipeline(pipeline_name):
    ...
    # In phase 3, we initialize the inversion's resolution and regularization coefficient, where we will:
    ...
    phase3 = InversionPhase(lens_galaxies=dict(lens=gm.GalaxyModel(light=lp.EllipticalSersic,
    ...
    # In phase 4, we fit the len galaxy light, mass and source galxy simultaneously, using an inversion. We will:

    # 1) Intialize the priors of the lens galaxy and source galaxy from phases 1+2+3.
    # 2) Use a circular mask which includes the lens and source galaxy light.
    ...
    phase4 = InversionPhase(lens_galaxies=dict(lens=gm.GalaxyModel(light=lp.EllipticalSersic,
                                                                   mass=mp.EllipticalIsothermal,
                                                                   shear=mp.ExternalShear)),
                            source_galaxies=dict(source=gm.GalaxyModel(pixelization=pix.AdaptiveMagnification,
                                                                      regularization=reg.Constant)),
                            optimizer_class=nl.MultiNest, mask_function=mask_function_circular,
                            phase_name=pipeline_name + '/phase_4_inversion')

    phase4.optimizer.const_efficiency_mode = True
    phase4.optimizer.n_live_points = 60
    phase4.optimizer.sampling_efficiency = 0.4
