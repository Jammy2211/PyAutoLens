...
def make_galaxy_model_2(mapper, test_config):
    galaxy_model_2 = gp.GalaxyModel(variable_redshift=True, light_profile=light_profiles.EllipticalDevVaucouleurs,
...
def make_galaxy_model(mapper, test_config):
    galaxy_model_1 = gp.GalaxyModel(variable_redshift=True, light_profile=light_profiles.EllipticalDevVaucouleurs,
                                    mass_profile=mass_profiles.EllipticalCoredIsothermal, config=test_config)
...
class TestMassAndLightProfiles(object):
    ...
    def test_make_galaxy_from_variable_profile(self):
        galaxy_model = gp.GalaxyModel(profile=light_and_mass_profiles.EllipticalSersic)

        arguments = {
            galaxy_model.profile.centre.centre_0: 1.0,
            galaxy_model.profile.centre.centre_1: 2.0,
            galaxy_model.profile.axis_ratio: 0.4,
            galaxy_model.profile.phi: 5.0,
            galaxy_model.profile.intensity: 0.6,
            galaxy_model.profile.effective_radius: 0.7,
            galaxy_model.profile.sersic_index: 8.0,
            galaxy_model.profile.mass_to_light_ratio: 3.0
        }
...
class TestGalaxyModel:
    ...
    def test_align_centres(self, galaxy_model, test_config, limit_config):
        prior_models = galaxy_model.prior_models

        assert prior_models[0].centre != prior_models[1].centre

        galaxy_model = gp.GalaxyModel(variable_redshift=True, light_profile=light_profiles.EllipticalDevVaucouleurs,
                                      mass_profile=mass_profiles.EllipticalCoredIsothermal,
    ...
    def test_align_axis_ratios(self, galaxy_model, test_config, limit_config):
        prior_models = galaxy_model.prior_models
    ...
    def test_align_phis(self, galaxy_model, test_config, limit_config):
        prior_models = galaxy_model.prior_models
...
class TestNamedProfiles:
    def test_get_prior_model(self):
        galaxy_model = gp.GalaxyModel(variable_redshift=True, light_profile=light_profiles.EllipticalSersic,
                                      mass_profile=mass_profiles.EllipticalSersic)

        assert isinstance(galaxy_model.light_profile, mm.PriorModel)
        assert isinstance(galaxy_model.mass_profile, mm.PriorModel)
    ...
    def test_set_prior_model(self):
        ...
        galaxy_model.light_profile = mm.PriorModel(light_profiles.LightProfile)
...
class TestResultForArguments:
    ...
    def test_complicated_instance_for_arguments(self):
        galaxy_model = gp.GalaxyModel(variable_redshift=True, align_centres=True,
        ...
        arguments = {galaxy_model.redshift.redshift: 0.5,
                     ...
                     galaxy_model.mass_profile.centre.centre_1: 2.0,
                     galaxy_model.mass_profile.einstein_radius: 3.0,
                     galaxy_model.light_profile.axis_ratio: 0.4,
                     galaxy_model.light_profile.phi: 5.0,
    ...
    def test_gaussian_prior_model_for_arguments(self):
        ...
        arguments = {galaxy_model.redshift.redshift: redshift_prior,
                     ...
                     galaxy_model.light_profile.effective_radius: mm.GaussianPrior(8, 1),
        ...
        assert gaussian_galaxy_model_model.mass_profile.einstein_radius == einstein_radius_prior
...
class TestPixelization(object):
    ...
    def test_pixelization(self, test_config):
        galaxy_model = gp.GalaxyModel(variable_redshift=True, pixelization=pixelizations.Rectangular,
...
class TestRegularization(object):
    ...
    def test_fixed_regularization(self, test_config):
        galaxy_model = gp.GalaxyModel(variable_redshift=True, pixelization=pixelizations.Voronoi(),
                                      regularization=regularization.Constant(), config=test_config)
...
class TestHyperGalaxy(object):
    def test_hyper_galaxy(self, test_config):
        galaxy_model = gp.GalaxyModel(variable_redshift=True, hyper_galaxy=g.HyperGalaxy, config=test_config)
...
class TestFixedProfiles(object):
    ...
    def test_fixed_mass_property(self):
        galaxy_model = gp.GalaxyModel(variable_redshift=True, mass_profile=mass_profiles.SphericalNFW())

        assert len(galaxy_model.constant_mass_profiles) == 1

    def test_fixed_mass(self):
        galaxy_model = gp.GalaxyModel(variable_redshift=True, nass_profile=mass_profiles.SphericalNFW())
    ...
    def test_fixed_and_variable(self):
        ...
        arguments = {galaxy_model.redshift.redshift: 0.2,
                     ...
                     galaxy_model.variable_light.intensity: 0.6,
                     galaxy_model.variable_light.effective_radius: 0.7,
...
class TestRedshift(object):
    def test_set_redshift_class(self):
        ...
        assert galaxy_model.redshift.redshift == 3
    ...
    def test_set_redshift_constant(self):
        galaxy_model = gp.GalaxyModel(variable_redshift=True, )
        galaxy_model.redshift = mm.Constant(3)
        # noinspection PyUnresolvedReferences
        assert galaxy_model.redshift.redshift == 3
