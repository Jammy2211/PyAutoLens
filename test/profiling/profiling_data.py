...
def load_data(name, pixel_scale, psf_shape):
    im = scaled_array.ScaledSquarePixelArray.from_fits_with_scale(file_path=path + 'data/' + name + '/masked_image', hdu=0,
                                                                  pixel_scale=pixel_scale)
    noise = scaled_array.ScaledSquarePixelArray.from_fits_with_scale(file_path=path + 'data/' + name + '/noise_map', hdu=0,
                                                                     pixel_scale=pixel_scale)
    exposure_time = scaled_array.ScaledSquarePixelArray.from_fits_with_scale(file_path=path + 'data/' + name + '/exposure_time',
                                                                             hdu=0,
                                                                             pixel_scale=pixel_scale)
