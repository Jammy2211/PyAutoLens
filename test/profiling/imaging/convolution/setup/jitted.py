...
lsst_convolver = Convolver(mask=lsst.mask, blurring_mask=lsst.masked_image.blurring_mask,
                           kernel=lsst.image_plane_images_.psf.trim_around_centre(psf_shape))
...
def euclid_solution():
    Convolver(mask=euclid.mask, blurring_mask=euclid.masked_image.blurring_mask,
              kernel=euclid.image_plane_images_.psf.trim_around_centre(psf_shape))


@tools.tick_toc_x1
def hst_solution():
    Convolver(mask=hst.mask, blurring_mask=hst.masked_image.blurring_mask,
              kernel=hst.image_plane_images_.psf.trim_around_centre(psf_shape))


@tools.tick_toc_x1
def hst_up_solution():
    Convolver(mask=hst_up.mask, blurring_mask=hst_up.masked_image.blurring_mask,
              kernel=hst_up.image_plane_images_.psf.trim_around_centre(psf_shape))


@tools.tick_toc_x1
def ao_solution():
    Convolver(mask=ao.mask, blurring_mask=ao.blurring_mask, kernel=ao.image_plane_images_.psf.trim_around_centre(psf_shape))
