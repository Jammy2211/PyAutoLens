...
def simulate_intensities(data_name, pixel_scale, galaxy):
    ...
    intensities = imaging_util.map_unmasked_1d_array_to_2d_array_from_array_1d_and_shape(array_1d=intensities,
                                                                                             shape=image_shape)

    if os.path.exists(output_path) == False:
        os.makedirs(output_path)

    imaging_util.numpy_array_to_fits(intensities, path=data_path + data_name +'.fits', overwrite=True)
