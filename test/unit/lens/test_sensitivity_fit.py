...
class TestSensitivityProfileFit:
    ...
    def test__tracer_and_tracer_sensitive_are_identical__added__likelihood_is_noise_term(self, ld_blur):
        ...
        fit = sensitivity_fit.SensitivityProfileFit(lens_data=ld_blur, tracer_normal=tracer,
        ...
        residual_map_1d = fit_util.residual_map_from_data_mask_and_model_data(
            data=ld_blur.image_1d, mask=ld_blur.mask_1d, model_data=model_image_1d)

        residual_map_2d = ld_blur.map_to_scaled_array(array_1d=residual_map_1d)
    ...
    def test__tracers_are_different__likelihood_is_non_zero(self, ld_blur):
        ...
        fit = sensitivity_fit.SensitivityProfileFit(lens_data=ld_blur, tracer_normal=tracer_normal,
        ...
        chi_squared_map_2d = fit_util.chi_squared_map_from_residual_map_noise_map_and_mask(
            residual_map=residual_map_2d, mask=ld_blur.mask_2d, noise_map=ld_blur.noise_map_2d)
        ...
        fit_from_factory = sensitivity_fit.fit_lens_data_with_sensitivity_tracers(lens_data=ld_blur,
...
class TestSensitivityInversionFit:
    ...
    def test__tracer_and_tracer_sensitive_are_identical__added__likelihood_is_noise_term(self, ld_blur):
        ...
        fit = sensitivity_fit.SensitivityInversionFit(lens_data=ld_blur, tracer_normal=tracer,
                                                      tracer_sensitive=tracer)
        ...
        assert fit.fit_normal.model_image_2d == pytest.approx(inversion.reconstructed_data_2d, 1.0e-4)

        residual_map_2d = fit_util.residual_map_from_data_mask_and_model_data(
            data=ld_blur.image_2d, mask=ld_blur.mask_2d, model_data=inversion.reconstructed_data_2d)
        ...
        assert fit.fit_normal.chi_squared_map_2d == pytest.approx(chi_squared_map_2d, 1.0e-4)

        assert fit.fit_sensitive.image_2d == pytest.approx(ld_blur.image_2d, 1.0e-4)
        assert fit.fit_sensitive.noise_map_2d == pytest.approx(ld_blur.noise_map_2d, 1.0e-4)
        assert fit.fit_sensitive.model_image_2d == pytest.approx(inversion.reconstructed_data_2d, 1.0e-4)
        assert fit.fit_sensitive.residual_map_2d == pytest.approx(residual_map_2d, 1.0e-4)
        assert fit.fit_sensitive.chi_squared_map_2d == pytest.approx(chi_squared_map_2d, 1.0e-4)

        chi_squared = fit_util.chi_squared_from_chi_squared_map_and_mask(chi_squared_map=chi_squared_map_2d,
                                                                         mask=ld_blur.mask_2d)
        noise_normalization = fit_util.noise_normalization_from_noise_map_and_mask(mask=ld_blur.mask_2d,
    ...
    def test__tracers_are_different__likelihood_is_non_zero(self, ld_blur):
        ...
        fit = sensitivity_fit.SensitivityInversionFit(lens_data=ld_blur, tracer_normal=tracer_normal,
                                                      tracer_sensitive=tracer_sensitive)
