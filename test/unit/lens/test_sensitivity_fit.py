...
def make_sersic():
    return lp.EllipticalSersic(
        axis_ratio=1.0, phi=0.0, intensity=1.0, effective_radius=0.6, sersic_index=4.0
    )
...
class TestSensitivityProfileFit:
    def test__tracer_and_tracer_sensitive_are_identical__added__likelihood_is_noise_term(
        ...
        fit = sensitivity_fit.SensitivityProfileFit(
            lens_data=ld_blur, tracer_normal=tracer, tracer_sensitive=tracer
        )

        assert (
            fit.fit_normal.image(return_in_2d=True) == ld_blur.image(return_in_2d=True)
        ).all()
        assert (
            fit.fit_normal.noise_map(return_in_2d=True)
            == ld_blur.noise_map(return_in_2d=True)
        ).all()
        ...
        residual_map_2d = ld_blur.scaled_array_2d_from_array_1d(
        ...
        assert (
            fit.fit_normal.chi_squared_map(return_in_2d=True) == chi_squared_map_2d
        ).all()

        assert (
            fit.fit_sensitive.image(return_in_2d=True)
            == ld_blur.image(return_in_2d=True)
        ).all()
        assert (
            fit.fit_sensitive.noise_map(return_in_2d=True)
            == ld_blur.noise_map(return_in_2d=True)
        ).all()
        assert (
            fit.fit_sensitive.model_image(return_in_2d=True) == model_image_2d
        ).all()
        assert (
            fit.fit_sensitive.residual_map(return_in_2d=True) == residual_map_2d
        ).all()
        assert (
            fit.fit_sensitive.chi_squared_map(return_in_2d=True) == chi_squared_map_2d
        ).all()
    ...
    def test__tracers_are_different__likelihood_is_non_zero(self, ld_blur):
        ...
        g0_subhalo = g.Galaxy(
            redshift=0.5, subhalo=mp.SphericalIsothermal(einstein_radius=0.1)
        )
        g1 = g.Galaxy(redshift=1.0, light_profile=lp.EllipticalSersic(intensity=2.0))
        ...
        residual_map_2d = af.fit_util.residual_map_from_data_mask_and_model_data(
            data=ld_blur.image(return_in_2d=True),
            mask=ld_blur.mask_2d,
            model_data=model_image_2d,
        )
        ...
        chi_squared_normal = af.fit_util.chi_squared_from_chi_squared_map_and_mask(
            chi_squared_map=fit.fit_normal.chi_squared_map(return_in_2d=True),
            mask=ld_blur.mask_2d,
        )
        chi_squared_sensitive = af.fit_util.chi_squared_from_chi_squared_map_and_mask(
            chi_squared_map=fit.fit_sensitive.chi_squared_map(return_in_2d=True),
            mask=ld_blur.mask_2d,
        )
        noise_normalization = af.fit_util.noise_normalization_from_noise_map_and_mask(
            mask=ld_blur.mask_2d, noise_map=ld_blur.noise_map(return_in_2d=True)
        ...
        assert (
            fit.figure_of_merit
            == fit.fit_sensitive.likelihood - fit.fit_normal.likelihood
        )
...
class TestSensitivityInversionFit:
    def test__tracer_and_tracer_sensitive_are_identical__added__likelihood_is_noise_term(
        ...
        g1 = g.Galaxy(
            redshift=1.0, pixelization=pixelization, regularization=regularization
        )
        ...
        assert fit.fit_normal.residual_map(return_in_2d=True) == pytest.approx(
            residual_map_2d, 1.0e-4
        )
        ...
        assert fit.fit_normal.chi_squared_map(return_in_2d=True) == pytest.approx(
            chi_squared_map_2d, 1.0e-4
        )

        assert fit.fit_sensitive.image(return_in_2d=True) == pytest.approx(
            ld_blur.image(return_in_2d=True), 1.0e-4
        )
        assert fit.fit_sensitive.noise_map(return_in_2d=True) == pytest.approx(
            ld_blur.noise_map(return_in_2d=True), 1.0e-4
        )
        assert fit.fit_sensitive.model_image(return_in_2d=True) == pytest.approx(
            inversion.reconstructed_data_2d, 1.0e-4
        )
        assert fit.fit_sensitive.residual_map(return_in_2d=True) == pytest.approx(
            residual_map_2d, 1.0e-4
        )
        assert fit.fit_sensitive.chi_squared_map(return_in_2d=True) == pytest.approx(
            chi_squared_map_2d, 1.0e-4
        )
