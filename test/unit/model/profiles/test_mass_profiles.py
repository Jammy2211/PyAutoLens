...
class TestCoredPowerLaw(object):
    ...
    def test__deflections__correct_values(self):
        ...
        cored_power_law = mp.EllipticalCoredPowerLaw(centre=(-0.7, 0.5), axis_ratio=0.7, phi=60.0,
                                                     einstein_radius=1.3, slope=1.8, core_radius=0.2)
        deflections = cored_power_law.deflections_from_grid(grid=np.array([[0.1625, 0.1625]]))
    ...
    def test__deflections__change_geometry(self):
        cored_power_law_0 = mp.SphericalCoredPowerLaw(centre=(0.0, 0.0))
        cored_power_law_1 = mp.SphericalCoredPowerLaw(centre=(1.0, 1.0))
        deflections_0 = cored_power_law_0.deflections_from_grid(grid=np.array([[1.0, 1.0]]))
        deflections_1 = cored_power_law_1.deflections_from_grid(grid=np.array([[0.0, 0.0]]))
        assert deflections_0[0, 0] == pytest.approx(-deflections_1[0, 0], 1e-5)
        assert deflections_0[0, 1] == pytest.approx(-deflections_1[0, 1], 1e-5)

        cored_power_law_0 = mp.SphericalCoredPowerLaw(centre=(0.0, 0.0))
        cored_power_law_1 = mp.SphericalCoredPowerLaw(centre=(0.0, 0.0))
        deflections_0 = cored_power_law_0.deflections_from_grid(grid=np.array([[1.0, 0.0]]))
        deflections_1 = cored_power_law_1.deflections_from_grid(grid=np.array([[0.0, 1.0]]))
        assert deflections_0[0, 0] == pytest.approx(deflections_1[0, 1], 1e-5)
        assert deflections_0[0, 1] == pytest.approx(deflections_1[0, 0], 1e-5)
    ...
    def test__multiple_coordinates_in__multiple_quantities_out(self):
        ...
        cored_power_law = mp.EllipticalCoredPowerLaw(centre=(-0.7, 0.5), axis_ratio=0.7, phi=60.0,
                                                     einstein_radius=1.3, slope=1.8, core_radius=0.2)
        deflections = cored_power_law.deflections_from_grid(grid=np.array([[0.1625, 0.1625], [0.1625, 0.1625]]))
        assert deflections[0, 0] == pytest.approx(0.9869, 1e-3)
        assert deflections[0, 1] == pytest.approx(-0.54882, 1e-3)
    ...
    def test__summarize_in_units(self):
        ...
        assert summary_text[i] == 'pl_einstein_radius                                1.00 arcsec' ; i += 1
        assert summary_text[i] == 'pl_einstein_mass                                  3.1416e+00 angular' ; i += 1
        assert summary_text[i] == 'pl_mass_within_10.00_arcsec                       3.1416e+01 angular' ; i += 1
        assert summary_text[i] == 'pl_mass_within_500.00_arcsec                      1.5708e+03 angular' ; i += 1
...
class TestCoredIsothermal(object):
    ...
    def test__deflections__correct_values(self):
        ...
        cored_isothermal = mp.EllipticalCoredIsothermal(centre=(-0.7, 0.5), axis_ratio=0.7, phi=60.0,
                                                        einstein_radius=1.3, core_radius=0.2)
        deflections = cored_isothermal.deflections_from_grid(grid=np.array([[0.1625, 0.1625]]))
...
class TestIsothermal(object):
    ...
    def test__deflections__correct_values(self):
        ...
        deflections = isothermal.deflections_from_grid(grid=np.array([[0.1625, 0.1625]]))
...
class TestAbstractNFW(object):
    ...
    def test__rho_at_scale_radius__numerical_values_in_default_units(self):
        cosmology = mock_cosmology.MockCosmology(kpc_per_arcsec=2.0, critical_surface_density=2.0)
    ...
    def test__rho_at_scale_radius__unit_conversions(self):
        ...
        cosmology = mock_cosmology.MockCosmology(arcsec_per_kpc=0.25, kpc_per_arcsec=4.0, critical_surface_density=2.0)

        rho = nfw_arcsec.rho_at_scale_radius_for_units(unit_length='kpc', unit_mass='solMass', redshift_profile=0.5,
                                                       redshift_source=1.0, cosmology=cosmology)
        assert rho == pytest.approx(0.5 / 4.0, 1e-3)

        cosmology = mock_cosmology.MockCosmology(arcsec_per_kpc=0.25, kpc_per_arcsec=4.0, critical_surface_density=4.0)
...
class TestTruncatedNFW(object):
    ...
    def test_summarize_in_units(self):
        ...
        assert summary_text[i] == 'nfw_einstein_radius                               15.38 kpc' ; i += 1
        assert summary_text[i] == 'nfw_einstein_mass                                 1.4418e+12 solMass' ; i += 1
        assert summary_text[i] == 'nfw_mass_within_10.00_kpc                         5.2061e+12 solMass' ; i += 1
        assert summary_text[i] == 'nfw_mass_within_500.00_kpc                        7.3287e+12 solMass' ; i += 1
        assert summary_text[i] == 'nfw_rho_at_scale_radius                           29027857.02 solMass/kpc3' ; i += 1
        assert summary_text[i] == 'nfw_delta_concentration                           110665.28' ; i += 1
        assert summary_text[i] == 'nfw_concentration                                 14.40' ; i += 1
        assert summary_text[i] == 'nfw_radius_at_200x_cosmic_density                 481.41 kpc' ; i += 1
...
class TestSersic(object):
    ...
    def test__deflections_correct_values(self):
        ...
        sersic = mp.EllipticalSersic(centre=(-0.4, -0.2), axis_ratio=0.8, phi=110.0, intensity=5.0,
                                     effective_radius=0.2, sersic_index=2.0, mass_to_light_ratio=1.0)
        deflections = sersic.deflections_from_grid(grid=np.array([[0.1625, 0.1625], [0.1625, 0.1625]]))
        assert deflections[0, 0] == pytest.approx(1.1446, 1e-3)
        assert deflections[0, 1] == pytest.approx(0.79374, 1e-3)
...
class TestExponential(object):
    ...
    def test__deflections_correct_values(self):
        ...
        deflections = exponential.deflections_from_grid(grid=np.array([[0.1625, 0.1625]]))
...
class TestSersicMassRadialGradient(object):
    ...
    def test__compare_to_sersic(self):
        ...
        sersic_deflections = sersic.deflections_from_grid(grid=np.array([[0.1625, 0.1625]]))

        exponential = mp.EllipticalExponential(centre=(-0.4, -0.2), axis_ratio=0.8, phi=110.0, intensity=5.0,
                                               effective_radius=0.2, mass_to_light_ratio=1.0)
        exponential_deflections = exponential.deflections_from_grid(grid=np.array([[0.1625, 0.1625]]))

        assert sersic_deflections[0, 0] == exponential_deflections[0, 0] == pytest.approx(0.90493, 1e-3)
        assert sersic_deflections[0, 1] == exponential_deflections[0, 1] == pytest.approx(0.62569, 1e-3)
        ...
        assert sersic_deflections[0, 0] == dev_deflections[0, 0] == pytest.approx(-24.528, 1e-3)
        assert sersic_deflections[0, 1] == dev_deflections[0, 1] == pytest.approx(-3.37605, 1e-3)

        sersic_grad = mp.EllipticalSersicRadialGradient(centre=(-0.4, -0.2), axis_ratio=0.8, phi=110.0, intensity=5.0,
                                                        effective_radius=0.2, sersic_index=2.0, mass_to_light_ratio=1.0,
                                                        mass_to_light_gradient=0.0)
        sersic_grad_deflections = sersic_grad.deflections_from_grid(grid=np.array([[0.1625, 0.1625]]))

        sersic = mp.EllipticalSersic(centre=(-0.4, -0.2), axis_ratio=0.8, phi=110.0, intensity=5.0,
                                     effective_radius=0.2, sersic_index=2.0, mass_to_light_ratio=1.0)
        sersic_deflections = sersic.deflections_from_grid(grid=np.array([[0.1625, 0.1625]]))

        assert sersic_grad_deflections[0, 0] == sersic_deflections[0, 0] == pytest.approx(1.1446, 1e-3)
        assert sersic_grad_deflections[0, 1] == sersic_deflections[0, 1] == pytest.approx(0.79374, 1e-3)
...
class TestDeflectionsViaPotential(object):
    ...
    def test__compare_sie_at_phi_45__deflections_via_potential_and_calculation__reg_grid(self):
    ...
    def test__compare_sie_at_phi_0__deflections_via_potential_and_calculation_reg_grid(self):
    ...
    def test__compare_sis_deflections_via_potential_and_calculation__sub_grid(self):
    ...
    def test__compare_sie_at_phi_45__deflections_via_potential_and_calculation__sub_grid(self):
    ...
    def test__compare_sie_at_phi_0__deflections_via_potential_and_calculation_sub_grid(self):
    ...
    def test__sub_grid_binning(self):
        ...
        pixel_1_from_av_sub_grid = (deflections_sub_grid[0] + deflections_sub_grid[1]
                                          + deflections_sub_grid[2] + deflections_sub_grid[3])/4
...
class TestConvergenceViaJacobian(object):
    ...
    def test__compare_sie_at_phi_45__convergence_via_jacobian_and_calculation__reg_grid(self):
    ...
    def test__compare_sie_at_phi_0__convergence_via_jacobian_and_calculation__reg_grid(self):
    ...
    def test__compare_sis_convergence_via_jacobian_and_calculation__sub_grid(self):
    ...
    def test__compare_sie_at_phi_45__convergence_via_jacobian_and_calculation__sub_grid(self):
    ...
    def test__compare_sie_at_phi_0__convergence_via_jacobian_and_calculation__sub_grid(self):
    ...
    def test_convergence_sub_grid_binning(self):
        ...
        pixel_1_from_av_sub_grid = (convergence_sub_grid[0] + convergence_sub_grid[1]
                                    + convergence_sub_grid[2] + convergence_sub_grid[3]) / 4

        assert pixel_1_reg_grid == pytest.approx(pixel_1_from_av_sub_grid, 1e-4)

        pixel_10000_reg_grid = convergence_binned_reg_grid[9999]

        pixel_10000_from_av_sub_grid = (convergence_sub_grid[39999] + convergence_sub_grid[39998]
...
class TestJacobianandMagnification(object):
    ...
    def test__compare_magnification_from_eigenvalues_and_from_determinant__reg_grid(self):
        ...
        mean_error = np.mean(mag_via_determinant-mag_via_eigenvalues)

        assert mean_error < 1e-4

    def test__compare_magnification_from_determinant_and_from_convergence_and_shear__reg_grid(self):
    ...
    def test__compare_magnification_from_eigenvalues_and_from_determinant__sub_grid(self):
    ...
    def test__compare_magnification_from_determinant_and_from_convergence_and_shear__sub_grid(self):
    ...
    def test__Jacobian_sub_grid_binning(self):
        ...
        pixel_1_from_av_sub_grid = (A11_sub_grid[0] + A11_sub_grid[1]
                                          + A11_sub_grid[2] + A11_sub_grid[3])/4

        assert pixel_1_reg_grid == pytest.approx(pixel_1_from_av_sub_grid, 1e-4)

        pixel_10000_reg_grid = A11_binned_reg_grid[9999]

        pixel_10000_from_av_sub_grid = (A11_sub_grid[39999] + A11_sub_grid[39998]
                                        + A11_sub_grid[39997] + A11_sub_grid[39996])/4
    ...
    def test_shear_sub_grid_binning(self):
        ...
        pixel_1_from_av_sub_grid = (shear_sub_grid[0] + shear_sub_grid[1]
                                    + shear_sub_grid[2] + shear_sub_grid[3]) / 4

        assert pixel_1_reg_grid == pytest.approx(pixel_1_from_av_sub_grid, 1e-4)

        pixel_10000_reg_grid = shear_binned_reg_grid[9999]

        pixel_10000_from_av_sub_grid = (shear_sub_grid[39999] + shear_sub_grid[39998]
                                        + shear_sub_grid[39997] + shear_sub_grid[39996]) / 4
    ...
    def test_lambda_t_sub_grid_binning(self):
        ...
        pixel_1_from_av_sub_grid = (lambda_t_sub_grid[0] + lambda_t_sub_grid[1]
                                    + lambda_t_sub_grid[2] + lambda_t_sub_grid[3]) / 4

        assert pixel_1_reg_grid == pytest.approx(pixel_1_from_av_sub_grid, 1e-4)

        pixel_10000_reg_grid = lambda_t_binned_reg_grid[9999]

        pixel_10000_from_av_sub_grid = (lambda_t_sub_grid[39999] + lambda_t_sub_grid[39998]
                                        + lambda_t_sub_grid[39997] + lambda_t_sub_grid[39996]) / 4
    ...
    def test_lambda_r_sub_grid_binning(self):
        ...
        pixel_1_from_av_sub_grid = (lambda_r_sub_grid[0] + lambda_r_sub_grid[1]
                                    + lambda_r_sub_grid[2] + lambda_r_sub_grid[3]) / 4

        assert pixel_1_reg_grid == pytest.approx(pixel_1_from_av_sub_grid, 1e-4)

        pixel_10000_reg_grid = lambda_r_binned_reg_grid[9999]

        pixel_10000_from_av_sub_grid = (lambda_r_sub_grid[39999] + lambda_r_sub_grid[39998]
                                        + lambda_r_sub_grid[39997] + lambda_r_sub_grid[39996]) / 4
...
class TestCriticalCurvesandCaustics(object):
    ...
    def test__critical_curves_SIS__sub_grid(self):
    ...
    def test_caustics_SIS__sub_grid(self):
        ...
        caustic_tan = caustics[1]

        xcaustic_tan, ycaustic_tan = caustic_tan[:, 1], caustic_tan[:, 0]

        assert xcaustic_tan ** 2 + ycaustic_tan ** 2 == pytest.approx(sis.einstein_radius ** 2, 1e-1)

    def test_compare_tangential_critical_curves_from_magnification_and_lamda_t__reg_grid(self):
    ...
    def test_compare_tangential_critical_curves_from_magnification_and_lambda_t__sub_grid(self):
   ...
   # def test_compare_radial_critical_curves_from_magnification_and_lamda_r__sub_grid(self):
    ...
    def test_compare_tangential_caustic_from_magnification_and_lambda_t__sub_grid(self):
